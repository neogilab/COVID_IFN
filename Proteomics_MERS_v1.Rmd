...
```{r}
name_ana <- "MERS"
input <- "data/MERS/20200818_L1_nUT2_MERS_TMT_Second_experiment.csv"
n_c1 <- 3
n_c2 <- 3
# define conditions
c1 <- "UNINF"
...
samples$condition <- ifelse(str_detect(samples$names, "MERS"), "MERS","UNINF")
...
```{r}
a <- autoplot(pca, data = pca_data, size= 6, shape = 21, fill = 'group')+  scale_fill_manual(values = c(c_c2, c_c1))+theme_gray()
print(a)
...
#72617d
```{r}
b <- autoplot(pca, data = pca_data, size= 6, shape = 21, fill = 'group')+ scale_fill_manual(values = c(c_c2, c_c1))+theme_gray()
print(b)
save_figure(b, "pca_after_norm_", 4, 4)
```

```{r}
c <- autoplot(pca, data = pca_data, size= 6, fill = 'group', label = TRUE, label.size = 3, shape = FALSE)+ scale_fill_manual(values = c(c_c2, c_c1))+theme_gray()
